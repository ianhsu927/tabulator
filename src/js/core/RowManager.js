...
export default class RowManager extends CoreFeature {
	...
	constructor(table) {
		...
		this.element = this.createHolderElement(); // 包含元素
		this.tableElement = this.createTableElement(); // 表格元素
		this.heightFixer = this.createTableElement(); //table element?
		this.placeholder = null; // 表格占位符
		this.placeholderContents = null; // 表格占位元素

		this.firstRender = false; // 处理第一次渲染
		this.renderMode = "virtual"; // 现有的渲染模式
		this.fixedHeight = false; //current rendering mode?

		this.rows = []; //hold 行数据对象
		this.activeRowsPipeline = []; // hold calculation of active rows
		this.activeRows = []; // 显示在表格中 available 的行
		this.activeRowsCount = 0; // 显示在表格中的行总数
		...
		this.dataPipeline = []; // 存储数据管道任务
		this.displayPipeline = []; // 存储数据显示管道任务
